...
/*
    rbt是一种特殊的二叉查找树,满足任意节点的值大于左孩子值,小于右孩子值。
    
    红黑树的每个节点增加记录了父节点。

    红黑树在每个节点增加了一个存储位记录节点的颜色，可以是RED,也可以是BLACK；
    通过任意一条从根到叶子简单路径上颜色的约束，红黑树保证最长路径不超过最短路径的二倍，因而近似平衡。
    （根据特性5，全黑节点是最短路径，最长路径是交替的黑红节点）
    ...
    红黑树的基本操作是添加、删除和旋转（左旋和右旋）。

    插入节点：按照二叉搜索树的方法进行节点插入，并设置颜色为红色，插入节点后通过旋转、颜色调整来修正树。
    删除节点：按照二叉搜索树的方法进行节点删除，删除节点后通过旋转、颜色调整来修正树。

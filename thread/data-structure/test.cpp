...
void f6(){
    ...
    while(i-->0){
        ...
        // std::cout << tm.value_for( "M20207"+std::to_string(rand()%2560) , -1 ) << " , ";
        std::this_thread::sleep_for( std::chrono::milliseconds(0) );
    }
}
...
/**
  * @brief          lock_free_stack测试
  * @retval         none
  */
lock_free_stack< std::string > ls;

void f9(){
    int i = 100000;
    while(i-->0){
        ls.push( "M2020707"+std::to_string(rand()%64) );
        std::this_thread::sleep_for( std::chrono::milliseconds(0) );
    }
}
...
void lock_free_stack_test(){
    ...
    std::vector< std::thread > threads; // thread对象容器
    for( int i=0; i<50 ;i++){
        threads.push_back( std::thread(f9) );
        threads.push_back( std::thread(f10) );
    }
    std::for_each( threads.begin() ,threads.end() ,std::mem_fn( &std::thread::join ) ); // 轮流在每个线程上调用join
}
...
/**
  * @brief          lock_free_queue测试
  * @retval         none
  */
// lock_free_queue< std::string > lq;

// void f11(){
//     int i = 100000;
//     while(i-->0){
//         lq.push( "M2020707"+std::to_string(rand()%64) );
//         std::this_thread::sleep_for( std::chrono::milliseconds(0) );
//     }
// }
...
//     std::vector< std::thread > threads; // thread对象容器
//     for( int i=0; i<50 ;i++){
//         threads.push_back( std::thread(f11) );
//         threads.push_back( std::thread(f12) );
//     }
//     std::for_each( threads.begin() ,threads.end() ,std::mem_fn( &std::thread::join ) ); // 轮流在每个线程上调用join
// }

    ...
    */
    ...
    private:
        ...
        /*
            ...
            Value value_for() 通过boost::shared_lock<boost::shared_mutex>读写互斥元，调用find_entry_for查找key
            void add_or_update_mapping() 通过std::unique_lock<boost::shared_mutex>读写互斥元，先查找元素，然后执行添加/更新键值对
            void remove_mapping() 通过std::unique_lock<boost::shared_mutex>读写互斥元，先查找元素，然后执行删除
        */
